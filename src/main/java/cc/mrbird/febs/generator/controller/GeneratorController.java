...
public class GeneratorController extends BaseController {
    ...
    @Value("${generator.databsename}")
    private String databaseName;

    @GetMapping("generator/tables/info")
    @RequiresPermissions("generator:view")
    public FebsResponse tablesInfo(String tableName, QueryRequest request) {
        Map<String, Object> dataTable = getDataTable(generatorService.getTables(tableName, request, GeneratorConstant.DATABASE_TYPE, databaseName));
        return new FebsResponse().success().data(dataTable);
    }
    @GetMapping("/generator/tables")
    public FebsResponse tables() {
        List<Table> tables = generatorService.getTablesList(GeneratorConstant.DATABASE_TYPE, databaseName);
        return new FebsResponse().success().data(tables);
    }


    @GetMapping("/generator/columns")
    public FebsResponse columns(String tableName) {
        List<Column> columns = generatorService.getColumns(GeneratorConstant.DATABASE_TYPE, databaseName,tableName);
        return new FebsResponse().success().data(columns);
    }
